...
Static Function TIDevMan()
    ...
    DEFINE WINDOW oSelWnd FROM 0,0 TO 1000, 1000 TITLE "TIDev - Ferramenta de desenvolvimento [INTEGRADOR]"
        oSelWnd:oFont := oFont
    ...
    DEFINE WINDOW oMainWnd FROM 0,0 TO 1000, 1000 TITLE "TIDev - Ferramenta de desenvolvimento [INTEGRADOR]"
...
Static Function TIDevChild()
    ...
    DEFINE MSDIALOG oDlg FROM 0,0 TO oMainWnd:nHeight - 100, oMainWnd:nWidth  - 20  Pixel TITLE  "TIDev - Ferramenta de desenvolvimento [INTEGRADOR]"
        oDlg:lEscClose := .F.
    ...
    oPS:Align :=CONTROL_ALIGN_ALLCLIENT
        ...
        oPSR:lVisibleControl := .F.
            ...
            oPSRB:Align :=CONTROL_ALIGN_TOP
                ...
                oBPS10 := THButton():New(002, 062, "Embedded"        , oPSRB, {|| cMemoL := ToEmbedded(@cMemoR)    }, 80, 10, oFontB, "Converte Advpl (cQuery) para o formato Embedded") 
                oBPS11 := THButton():New(002, 142, "cQuery"          , oPSRB, {|| cMemoL := TocQuery(@cMemoR)      }, 80, 10, oFontB, "Converte Embedded para o formato Advpl (cQuery)") 
...
Static Function MenuQuery(cCMD, oOwner, np)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Excluir"         , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstQry[np], cSufix, cCMD, .F., .T. ), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstQry[np], cSufix, ""  , .T., .F. ), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function LimpaTmp()
    Local cDir1 := "system\tidev_INTEGRADOR\"
    Local cDir2:= "system\tidev_INTEGRADOR\ctreeint\"
    Local nx 
...
Static Function LimpaQry()
    Local np
    Local cAliasExqr
    Local cDir := "system\tidev_INTEGRADOR\"
...
Static Function FolderDic(oDlg, np) 
    ...
    oP1:Align :=CONTROL_ALIGN_TOP
        ...
        THButton():New(005, 390, "Edit Reg"   , oP1, {|| TelaEdit(cAliasDic, np)  }, 40, 10, oFontB, "Edita os campos")     
        THButton():New(020, 390, "Copy Reg"   , oP1, {|| TelaCopy(cAliasDic, np)  }, 40, 10, oFontB, "Edita os campos")     

        THButton():New(005, 430, "Filter"     , oP1, {|| FilterTab(cAliasDic, np)  }, 40, 10, oFontB, "Aplica filtro da tabela") 
        THButton():New(020, 430, "Replace"    , oP1, {|| ReplaceTab(cAliasDic, np) }, 40, 10, oFontB, "Altera registros da tabela") 
        
        THButton():New(005, 470, "Locate"     , oP1, {|| LocateTab(cAliasDic, np)  }, 40, 10, oFontB, "Localiza registro conforme expressão") 
        THButton():New(020, 470, "Goto"       , oP1, {|| GotoTab(cAliasDic, np)    }, 40, 10, oFontB, "Posiciona no recno") 
        
        THButton():New(005, 510, "Delete"     , oP1, {|| DeleteTab(cAliasDic, np, .T.)  }, 40, 10, oFontB, "Deleta registros da tabela") 
        THButton():New(020, 510, "Recall"     , oP1, {|| DeleteTab(cAliasDic, np, .F.)  }, 40, 10, oFontB, "Recupera registros deletados") 
        
        THButton():New(005, 550, "Import"     , oP1, {|| ImportTab(cAliasDic, np)  }, 40, 10, oFontB, "Importa registros de outra tabela") 
        THButton():New(020, 550, "Export"     , oP1, {|| ExportTab(cAliasDic, np)  }, 40, 10, oFontB, "Exporta registros para outra tabela") 
        
        THButton():New(005, 590, "Estrutura"  , oP1, {|| Estrutura(cAliasDic, np) }, 40, 10, oFontB, "Mostra a estrutura da tabela") 
        THButton():New(020, 590, "Colunas"    , oP1, {|| ReorgColu(cAliasDic, oP2, np, oIndice, oMsg , oMsg2)  }, 40, 10, oFontB, "Reorganize as colunas") 
        
        THButton():New(005, 630, "Compara SX" , oP1, {|| CompSX(cAliasDic, np)    }, 40, 10, oFontB, "Compara estrutura da tabela com o dicionario") 
        THButton():New(020, 630, "Sync SX3"   , oP1, {|| SincSX3(cAliasDic,  np)  }, 40, 10, oFontB, "Altera estrutura da tabela conforme estrutura do dicionario SX3") 
        
        THButton():New(005, 670, "Drop Index" , oP1, {|| DropInd(cAliasDic,  np)  }, 40, 10, oFontB, "Apaga os indices") 
...
Static Function TelaEdit(cAliasDic, np)
    ...
    If TIEnchoice(cAliasTst, cAliasDic, lReadOnly, lCopia)
        If ValType(__aDicBrw[np])=='O'
            __aDicBrw[np]:Refresh()
            __aDicBrw[np]:SetFocus()
            eval(__aDicBrw[np]:bChange)
...
Static Function TIEnchoice(cAlias, cTitulo, lReadOnly, lCopia)
    Local oModal   
    Local oDlg      
    Local oScroll  
    Local nLinha     := 8
    ...
    If lSalva .AND. ! lReadOnly
        ...
        If lCopia
            If ! MsgYesNo("Confirma a copia do registro " + Alltrim(Str((cAlias)->(RecNo()))) + "?")
                Return .F.
            EndIf 
            (cAlias)->(RecLock(cAlias, .T.))
        Else 
            If ! MsgYesNo("Confirma a atualização do registro " + Alltrim(Str((cAlias)->(RecNo())))+ "?")
                Return .F.
            EndIf 
...
Static Function SelArquivo(cFormato, cArquivo, cModo, oModal)
    ...
    Else
        ...
        If Subs(cfile, 2, 2) == ":\"    
            ...
            If File("system\tidev_INTEGRADOR\" + cNomArq) 
                lUsaAnt := MsgYesNo("Esse arquivo já existe em 'system\tidev_INTEGRADOR\', deseja reutilizar o arquivo?")
                If ! lUsaAnt
                    MsgAlert("Será feito uma copia temporaria no servido em 'system\tidev_INTEGRADOR\' para processamento!")
                    If FErase("system\tidev_INTEGRADOR\" + cNomArq) == -1
                        MsgAlert("o arquivo em 'system\tidev_INTEGRADOR' não pode ser substituido!")
                        Return .f.
                    EndIf 
                EndIf
            Else 
            ...
            EndIf 
            If ! lUsaAnt 
                Processa({|| lSucesso := CPYT2S(cFile, "system\tidev_INTEGRADOR\", .T.) }, "Copiando arquivo para \system\tidev_INTEGRADOR..")
                If ! lSucesso
                    MsgAlert("Erro ao copiar arquivo para 'system\tidev_INTEGRADOR\'")
                    REturn .f.
                EndIf
            EndIf
            cFile := "system\tidev_INTEGRADOR\" + cNomArq
        EndIf 
    EndIf 
    cArquivo := cFile
...
Static Function ImportTab(cAliasDic, np)
    ...
    ElseIf cFormato == "DTC"
        ...
        (cAliasTmp)->(DbCloseArea())
        If Left(cArquivo, 24) == "system\tidev_INTEGRADOR\"
            FErase(cArquivo)
        EndIf 
    ElseIf cFormato == "CSV"
...
Static Function MenuCmp(oLbx1, oLbx2, oOwner, cAliasDic)
    ...
    //oMenu:Add(tMenuItem():new(oMenu, "Excluir"         , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstCmp, "campo" + Alltrim(cAliasDic), cCMD, .F., .T.), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstCmp, "campo" + Alltrim(cAliasDic), ""  , .T., .F.), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function FolderRPO(oDlg)
    ...
    @ 005, 004 MSCOMBOBOX oTipo    VAR cTipo    ITEMS aTipo   SIZE 60,09 PIXEL OF oP1 VALID SelTipo(cTipo, oRPO, oSX3, oSX6, oSX7, oMNU ) 
    ...
    @ 005, 520 CHECKBOX oMNU VAR __lMnu  PROMPT "MNU"	 SIZE 025,010 OF oP1 PIXEL font oFontB 
...
Static Function SelTipo(cTipo, oRPO, oSX3, oSX6, oSX7, oMNU) 
...
Static Function FolderCmd(oDlg)
    ...
    oP1:Align :=CONTROL_ALIGN_TOP
        ...
        @ 002, 252 CHECKBOX oCheck VAR lCheck	 PROMPT "Trata Erro"	 SIZE 055,010 OF oP1 PIXEL font oFontB 
...
Static Function MenuCmd(cCMD, oOwner)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Excluir"         , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstCmd, "cmd", cCMD, .F., .T.), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstCmd, "cmd", ""  , .T., .F.), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function MenuBase(cCMD, oOwner)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Excluir"       , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstBase, "base", cCMD, .F., .T.), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Excluir todos" , , , , {|| If(MsgYesNo("Confirma a exclusão de toda base"), (GrvHst(__aHstBase, "base", ""  , .T., .F.), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function MenuHtm(cCMD, oOwner)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Excluir"         , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstHtm, "htm", cCMD, .F., .T.), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstHtm, "htm", ""  , .T., .F.), .t. ), .t.) }, , , , , , , , , .T.))
...
Static Function MenuFil(cFiltro, oOwner)
    ...
    oMenu:Add(tMenuItem():new(oMenu, "Excluir"         , , , , {|| If(MsgYesNo("Confirma a exclusão?")            , (GrvHst(__aHstFil, "filtro", cFiltro, .F., .T.), .t. ), .t.) }, , , , , , , , , .T.))
    oMenu:Add(tMenuItem():new(oMenu, "Limpar Histórico", , , , {|| If(MsgYesNo("Confirma a limpeza do histórico?"), (GrvHst(__aHstFil, "filtro", ""     , .T., .F.), .t. ), .t.) }, , , , , , , , , .T.))
            ...
            For nX:=1 To (cAlias)->(FCount())
                ...
                If cTipo == "N"
                    ...
                    Else
                        cPict := Repl("9", nTam ) 
                    EndIf 
                    __aQryBrw[np]:AddColumn( TCColumn():New( (cAliasTst)->(Field(nx)) , &("{ || " + cAlias + "->" + (cAlias)->(FieldName(nX)) + "}"), cPict,,,, "RIGHT"))
                ElseIf cTipo == "L"
                    __aQryBrw[np]:AddColumn( TCColumn():New( aStruct[nx, 1], &("{ || " + cAlias + "->" + (cAlias)->(FieldName(nX)) + "}"), "@!",,,, "LEFT"))
                ElseIf cTipo == "C"     
                    __aQryBrw[np]:AddColumn( TCColumn():New( aStruct[nx, 1], &("{ || " + cAlias + "->" + (cAlias)->(FieldName(nX)) + "}"), "@!",,,, "LEFT"))
                ElseIf cTipo == "D"
                    If Valtype((cAlias)->(FieldGet(nX))) == "D"
                        __aQryBrw[np]:AddColumn( TCColumn():New( aStruct[nx, 1], &("{ || " + cAlias + "->" + (cAlias)->(FieldName(nX)) + "}"), "@E",,,, "CENTER") )
                    Else 
                        __aQryBrw[np]:AddColumn( TCColumn():New( aStruct[nx, 1], &("{ || Stod(" + cAlias + "->" + (cAlias)->(FieldName(nX)) + ")}"), "@E",,,, "CENTER") )
                    EndIf 
                ElseIf cTipo == "M"
                    __aQryBrw[np]:AddColumn( TCColumn():New( aStruct[nx, 1], { || "Memo" },,,,,.F.) )
                EndIf 
