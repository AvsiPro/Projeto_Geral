...
varinfo("oRet - intENotas", oRet)
        ...
        ZPW->ZPW_LNKPDF := oRet["nfeLinkPdf"]
        ZPW->ZPW_LNKXML := oRet["nfeLinkXml"]
        ZPW->ZPW_NUMERO := oRet["nfeNumero"]
        ZPW->ZPW_CODVER := oRet["nfeCodigoVerificacao"]
...
// atualiza demais registros da NF p/ Last <- N
        ...
        elseif cStatus $ "SOLICITANDOCANCELAMENTO;CANCELAMENTOSOLICITADO;EMPROCESSODECANCELAMENTO;CANCELADAAGUARDANDOATUALIZACAOPDF"
            cStat := "6"        // Processo de Cancelamento
        elseif cStatus == 'CANCELAMENTONEGADO'
            cStat := "7"
        elseif cStatus == 'CANCELADA'
            cStat := "8"
        else
            cStat := "9"
        endif
