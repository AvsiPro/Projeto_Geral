...
If !oProcess:lEnd
	...
	ElseIf nLin > 0
		...
		U_BACCopy(cArqOut) // Cópia do arquivo para a máquina do usuário (TNU_CUSTOM)
	Else
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Validações customizadas - TOTVS Nações Unidas (TNU_CUSTOM)             ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If !U_BACValid()
    Return
EndIf
...
cQuery  += " AND RA_ADMISSA <= '" + cAdm + "' " + U_BAC02Query() // TNU_CUSTOM
...
If lNovoCalc
	...
	cSelect  += " RA_NASC,RA_CIC,RA_RG,RA_MAE,RA_EMAIL,RA_ESTCIVI,RA_SEXO,RA_PAI " + U_BAC01Query() // TNU_CUSTOM
	...
	cFrom  += " AND R0_NROPED >= '" + cPedDe + "' AND R0_NROPED <= '" + cPedAte + "' " + U_BAC02Query() // TNU_CUSTOM
...
Else
	...
	cQuery  += " RA_NASC,RA_CIC,RA_RG,RA_MAE,RA_EMAIL,RA_ESTCIVI,RA_SEXO,RA_PAI " + U_BAC01Query() // TNU_CUSTOM
	...
	cQuery  += " AND RG2_PERIOD = '" + cPeriodo  +"'"  + U_BAC02Query() // TNU_CUSTOM
...
While QD03VB->(!Eof())
    ...
    If lNovoCalc
		...
		cQuery  += " AND R0_NROPED >= '" + cPedDe + "' AND R0_NROPED <= '" + cPedAte + "' " + U_BAC03Query() // TNU_CUSTOM
    ...
    Else
		...
		If QD03VB->(RA_FILIAL+RA_MAT) <> cMyChave .And. U_BACVld01(QD03VB->RG2_DIACAL, QD03VB->RG2_CUSUNI) // TNU_CUSTOM
			If lOfuscaNom
				oProcess:IncRegua2(OEMTOANSI( STR0043 ) + ': ' + QD03VB->RA_MAT)
			else
				oProcess:IncRegua2(OEMTOANSI( STR0044 ) + ': ' + QD03VB->RA_NOME)
