...
User Function xCNTA301()
	...
	If ParamBox(aPergs ,"Transf. de Contrato Vigente Emp 020101 P/ 010101 ",aRet)      
		...
		If !CN9->(Eof())
			...
			MsgAlert("Array Planilha "+cValtoChar((cAliasCNA)->TOTREG))
			If (cAliasCNA)->TOTREG > 0 
				While !(cAliasCNA)->(Eof())		
					//MsgAlert("Total Planilha "+cValtoChar((cAliasCNA)->CNA_VLTOT+" SALDO: "+(cAliasCNA)->CNA_SALDO))	
			...
			If (cAliasCNB)->TOTREG > 0 
				While !(cAliasCNB)->(Eof())		
					...
					AADD(aCNB,(cAliasCNB)->CNB_DTCAD ) //14
					...
					AADD(aCNB,(cAliasCNB)->CNB_FILORI) //20
					AADD(aCNB,(cAliasCNB)->CNB_PROXRJ) //21
					AADD(aCNB,(cAliasCNB)->CNB_DTANIV) //22
					...
					AADD(aCNB,(cAliasCNB)->CNB_DTREAJ) //34
			...
			If (cAliasCPD)->TOTREG > 0 
				While !(cAliasCPD)->(Eof())
					aCPD := {}				
					AADD(aCNN,(cAliasCPD)->CPD_FILIAL)
					AADD(aCNN,(cAliasCPD)->CPD_CONTRA) 
					AADD(aCNN,(cAliasCPD)->CPD_NUMPLA) 
					AADD(aCNN,(cAliasCPD)->CPD_FILAUT) 
					AADD(aCPDFil,aCPD)
					(cAliasCPD)->(DbSkip())
				EndDo
...
User Function xCNTA300(aCN9,aCNC,aCNAPlan,aCNBItens,aCNNUsr,aCPDFil)
	...
	CN9->(dbSetOrder(1))
	If !(CN9->(dbSeek("010101"+cValToChar(aCN9[5]+PadR(aCN9[17],TamSx3("CN9_REVISA")[1])))))		
		//Cabeçalho do Contrato
		oModel:LoadValue('CN9MASTER','CN9_FILIAL',"010101") 
		oModel:LoadValue('CN9MASTER','CN9_TPCTO ',aCN9[2])
		oModel:LoadValue('CN9MASTER','CN9_DTINIC',dDataBase) 
		...
		For nCNA := 1 to Len(aCNAPlan)
			MsgAlert("CNA Planilha "+ aCNAPlan[nCNA][3]+" Nr:"+cValtoChar(nCNA))
			If nCNA > 1 
				//Cliente/Fornecedor do Contrato
				oModel:LoadValue('CNCDETAIL','CNC_CLIENT',aCNC[4]) 
				oModel:LoadValue('CNCDETAIL','CNC_LOJACL',aCNC[5])
			...
			//oModel:LoadValue('CNADETAIL','CNA_DTINI ',CtoD(aCNAPlan[nCNA][7]))
			oModel:LoadValue('CNADETAIL','CNA_DTINI ',dDataBase)
			oModel:LoadValue('CNADETAIL','CNA_VLTOT ',aCNAPlan[nCNA][8])
			oModel:LoadValue('CNADETAIL','CNA_SALDO ',aCNAPlan[nCNA][9])
			oModel:LoadValue('CNADETAIL','CNA_TIPPLA',aCNAPlan[nCNA][10])
			//oModel:LoadValue('CNADETAIL','CNA_DTFIM ',CtoD(aCNAPlan[nCNA][11]))
			oModel:LoadValue('CNADETAIL','CNA_DTFIM ',dDataBase)
			oModel:LoadValue('CNADETAIL','CNA_FLREAJ',aCNAPlan[nCNA][12])
			oModel:LoadValue('CNADETAIL','CNA_INDICE',aCNAPlan[nCNA][13])
			oModel:LoadValue('CNADETAIL','CNA_PROXRJ',CtoD(aCNAPlan[nCNA][14]))
			...
			oModel:LoadValue('CNADETAIL','CNA_DTREAJ',CtoD(aCNAPlan[nCNA][19]))
			...
			For nCNB := 1 to Len(aCNBItens) 
				If Alltrim(aCNBItens[nCNB,2]) == Alltrim(aCNAPlan[nCNA,3]) 
					MsgAlert("CNB Itens Planilha "+ aCNBItens[nCNB][2]+" Nr:"+cValtoChar(nCNB)+" Item"+aCNBItens[nCNB][4])
					...
					oModel:LoadValue('CNBDETAIL','CNB_ITEM  ',aCNBItens[nCNB][4])
					...
					oModel:LoadValue('CNBDETAIL','CNB_DTCAD ',CtoD(aCNBItens[nCNB][14])) 
					...
					oModel:LoadValue('CNBDETAIL','CNB_PROXRJ',CtoD(aCNBItens[nCNB][21]))
					oModel:LoadValue('CNBDETAIL','CNB_DTANIV',CtoD(aCNBItens[nCNB][22]))
					...
					//oModel:LoadValue('CNBDETAIL','CNB_RJRTO ',aCNBItens[nCNB][28])
					...
					oModel:LoadValue('CNBDETAIL','CNB_DTREAJ',CtoD(aCNBItens[nCNB][34]))
		...
		If oModel:VldData()
			...
			Next
			*/ 
			MsgAlert("Contrato Importado com Sucesso")
			//U_CN300SITU(Alltrim(CN9->CN9_NUMERO))
		Else
			lRet := .F.
		...
		Endif
	Else 
		MsgAlert("Contrato "+aCN9[5]+" já existe!")
	EndIf
	cFilAnt := cFilBkp 
Return lRet
