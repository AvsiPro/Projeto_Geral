...
STATIC function FILTRACTR()
	...
	BeginSql Alias cAliasCTR
        ...
        SELECT CN9_NUMERO, CN9_DTINIC, CN9_DTFIM, CN9_XMDCON, CNA_NUMERO, CNA_REVISA, CNA_CLIENT, CNA_LOJACL, A1_NOME, A1_CGC
...
STATIC function GRMEDCTR()
	...
	while (cTrbCTR)->( !EOF() )
		...
		if LEN(aMEDCTR) > 0  //.AND. Ltemqtd 
//			MSGINFO("aMEDCTR Passou aqui 01 ", "AVISO")
			VALIDACTR(aMEDCTR)
		endif
...
//RONI SILVA 04/10/2022 INICIO
//QUANDO TERMINAR DE LER OS CONTRATOS
if !Empty(cMsgBRSYST)
	U_zMsgLog(cMsgBRSYST , "RETORNO BRSYSTEM ")
endif

if !Empty(cMsgPROTHE)
//	U_zMsgLog(cMsgPROTHE , "RETORNO PROTHEUS ")
endif
...
//	LOCAL cMsg     := ''
	...
	if oRet:CMENSAGEMDERETORNO == "OK"
		...
		*/
	//RONI SILVA DEPOIS
	AADD( aRetorno,{ "NLIBERACOES"                             , oRet:NLIBERACOES                             , "SV0000000000020" } )
	AADD( aRetorno,{ "NPERIODOFATURAMENTOCOMPLEMENTARMOTORISTA", oRet:NPERIODOFATURAMENTOCOMPLEMENTARMOTORISTA, "SV0000000000047" } )
	AADD( aRetorno,{ "NPERIODOFATURAMENTOCOMUMMOTORISTA"       , oRet:NPERIODOFATURAMENTOCOMUMMOTORISTA       , "SV0000000000044" } )//"SV0000000000044" } )
	AADD( aRetorno,{ "NPERIODOFATURAMENTOVEICULO"              , oRet:NPERIODOFATURAMENTOVEICULO              , "SV0000000000044" } )//"SV0000000000044" } )
	AADD( aRetorno,{ "NPERIODOSOLICITANTECOMPLEMENTARMOTORISTA", oRet:NPERIODOSOLICITANTECOMPLEMENTARMOTORISTA, "SV0000000000004" } )
	AADD( aRetorno,{ "NPERIODOSOLICITANTECOMUMMOTORISTA"       , oRet:NPERIODOSOLICITANTECOMUMMOTORISTA       , "SV0000000000001" } )
	AADD( aRetorno,{ "NPERIODOSOLICITANTEVEICULO"              , oRet:NPERIODOSOLICITANTEVEICULO              , "SV0000000000001" } )
	AADD( aRetorno,{ "NTOTALVIAGENS"                           , oRet:NTOTALVIAGENS                           , "SV0000000000028" } )
	AADD( aRetorno,{ "NTOTALVIAGENS"                           , oRet:NTOTALVIAGENS                           , "SV0000000000029" } )
	AADD( aRetorno,{ "NVEICULOSFROTA"                          , oRet:NVEICULOSFROTA                          , "SV0000000000027" } )
	AADD( aRetorno,{ "NVIAGENSAVULSAS"                         , oRet:NVIAGENSAVULSAS                         , "SV0000000000086" } )
	AADD( aRetorno,{ "NVIAGENSTELEMONITORADAS"                 , oRet:NVIAGENSTELEMONITORADAS                 , "SV0000000000046" } )
	AADD( aRetorno,{ "NLIBERACOESTRANSPORTADORES"              , oRet:NLIBERACOESTRANSPORTADORES              , "SV0000000000021" } )
...
//	LOCAL cMsg       := ''
	...
	//--------------------------
//	MSGINFO("aProdCTR Passou aqui 01 ", "AVISO")
...
//		MSGINFO("aProdCTR for Passou aqui 01 ", "AVISO")

		cProduto := aProdCTR[nX][1]
		linc := .F.
		For nNx := 1 to len(aMedCTR)

//		MSGINFO("aMedCTR for Passou aqui 01 ", "AVISO")
...
//					MSGINFO("aProdCTR for Passou aqui 01 ", "AVISO")
					If Alltrim(aProdCTR[nCont][1]) == Alltrim(aMedCTR[nNx][3]) .AND. aMedCTR[nNx][2] > 0  //RONI SILVA 04/10/2022
						linc := .T.
		...
		If !lInc
//					MSGINFO("!lInc Passou aqui 01 ", "AVISO")		
...
//RONI SILVA COMENTADO PARA TRAZER TODOS OS PRODUTOS		If aProdutos[nNx][2] > 0 .or. aProdutos[nNx][5] > 0
		aadd(aProduto,{ aProdutos[nNx][1], aProdutos[nNx][2], aProdutos[nNx][3], aProdutos[nNx][4], aProdutos[nNx][5], aProdutos[nNx][6]})
//		EndIf	
	Next nNx

	if !lInc // LEN(aProduto) == 0
		//RONI SILVA 04/10/2022 INICIO
		...
		For nNx :=1 to len(aMedCTR)
			...
			ENDIF
		Next nNx
//		MSGALERT( "Produtos retornado na consulta não constam no contrato "+cNrContra, "AVISO" )
//		U_zMsgLog(cMsgPROTHE , "RETORNO PROTHEUS ")

		//RONI SILVA 04/10/2022 FIM
//		return()
	endif
	...
	//--------------------
//	MSGINFO("Inicia pelo CONTRATO Passou aqui 01 ", "AVISO")
	for nX := 1 to LEN(aIntWeb)
		...
		if !CN9->( DBSEEK( xFILIAL("CN9") + cNrContra + cNrRevisa ) )
			...
			if CN9->CN9_SITUAC <> "05"
				MSGINFO("Contrato não esta em VIGÊNCIA", "AVISO" )
				lRet := .F.
			endif
...
//					MSGINFO("Passou aqui 01 ", "AVISO")
		if lRet
			...
			if !CNA->( DBSEEK( xFILIAL("CNA") + CN9->CN9_NUMERO + CN9->CN9_REVISA + cNrPlan ) )
				...
				If CNL->CNL_CTRFIX <> '3'
					MSGINFO("Planilha: " + cNrPlan + " Não tipo 3", "AVISO")
					lRet := .F.
				EndIf
...
//						MSGINFO("Passou aqui 02 ", "AVISO")
						...
						If(oModel:CanActivate())
							...
							oModel:SetValue("CNDMASTER","CND_NUMERO"    ,cNrPlan       )
							...
							for nY := 1 to LEN(aProduto)   //oModel:GetModel("CXNDETAIL"):Length()   //LEN(aProduto)
								...
								//RONI SILVA INICIO LISTAR QUANTIDADE MINIMA
//			MSGINFO("cMsgBRSYST Passou aqui 01 ", "AVISO")								
								...
								//RONI SILVA TRATAR AQUI OS PRODUTOS CANCELADOS
								DbSelectArea("CNB")
								DbSetOrder(1)
...
//											MSGINFO("Passou aqui 03 ", "AVISO")
											EXIT
										ENDIF
									end
								endif

							oModel:GetModel("CXNDETAIL"):GoLine(nlinCXN)   //GoLine(VAL(ALLTRIM(cNrPlan)))  aProduto[nY][03]
							oModel:SetValue("CXNDETAIL","CXN_CHECK" , .T.)//Marcar a planilha(nesse caso apenas uma)

								//roni silva pre-cadastrao CNB
								nlinCNE  := oModel:GetModel("CNEDETAIL"):Length()
								oModel:GetModel('CNEDETAIL'):GoLine(val(nItem)) //nY
								oModel:SetValue("CNEDETAIL" , "CNE_NUMERO"  ,cNrPlan)
//								oModel:SetValue('CNEDETAIL' , 'CNE_ITEM'    , nItem)  //RONI SILVA 17/08/2022 ALTERADO PARA TRATAR O ITEM NA CNE
							...
							if ntotquant <> 0
								if nlinCXN >= oModel:GetModel("CXNDETAIL"):Length()
									nlinCXN := 1
								else
									nlinCXN++
								endif
							...
							If(oModel:HasErrorMessage())
								...
								Alert('Erro: ' + AllToChar( aErro[6] + ' '+ AllToChar( aErro[4] + Dtoc(DATE())+' - '+"Contrato: " +  cNrContra +" Planilha: "+cNrPlan +" Hora: "+Time())))
							Else
...
STATIC function VLDPRDCTR(cNrContra,cNrRevisa,cNrPlan )
	...
	while (cAliasCNB)->( !EOF() )
		...
		AADD( aRetorno2, { (cAliasCNB)->CNB_PRODUT, (cAliasCNB)->CNB_ITEM, (cAliasCNB)->CNB_VLUNIT, (cAliasCNB)->CNB_XQMIN, (cAliasCNB)->CNB_TABPRC })  // strzero(ncontaitem,3)

		//ncontaitem++

		(cAliasCNB)->( DBSKIP() )
//		MSGINFO("aRetorno2 Passou aqui 01 ", "AVISO")
	enddo
