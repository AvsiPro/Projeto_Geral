...
Static Function BRWSCTR(cA1CGC)
	...
	If oRet:CMENSAGEMDERETORNO == "OK"
		...
		(cAliasA)->(DbGoTop())
		While (cAliasA)->(!Eof())
			AADD( aRetorno,{ AllTrim((cAliasA)->ZZ5_BRSYST)	, &("oRet:" + cValToChar(AllTrim((cAliasA)->ZZ5_BRSYST))), AllTrim((cAliasA)->ZZ5_COD) } )
...
Static Function VALIDACTR(aMedCTR)
	...
	For nX := 1 to LEN( aProdCTR)
		...
		lInc := .F.
		nNx := Ascan(aMedCTR,{|x| x[3] == cProduto .And. x[2] > 0})
		If nNx > 0
			lInc := .T.
			If aProdCTR[nX][4] < aMedCTR[nNx][02]
				aProdCTR[nX][4] := aMedCTR[nNx][2]
			EndIF 
		EndIf 
		...
		//If !lInc
			AADD( aProdutos, { aProdCTR[nX][1], aProdCTR[nX][4], aProdCTR[nX][2], aProdCTR[nX][3], aProdCTR[nX][4], aProdCTR[nX][5], aProdCTR[nX][6], aProdCTR[nX][7]} )
		//EndIf
	next nX

	For nNx := 1 to len(aProdutos)
		aadd(aProduto,{ aProdutos[nNx][1], aProdutos[nNx][2], aProdutos[nNx][3], aProdutos[nNx][4], aProdutos[nNx][5], aProdutos[nNx][6], aProdutos[nNx][7], aProdutos[nNx][8]})
	Next nNx
		...
		If CN9->( DbSeek( xFilial("CN9") + cNrContra + cNrRevisa ) )
			...
			If(oModel:CanActivate())
				...
				For nY := 1 to LEN(aProduto)
					...
					cNumero  := aProduto[nY][07] //aqui
					cItemCNB := aProduto[nY][08] //aqui
					...
					CNB->(DbSetOrder(1))
					//If CNB->( DbSeek( xFilial("CNB") + cNrContra + cNrRevisa ) ) //aqui
					If CNB->( DbSeek( xFilial("CNB") + cNrContra + cNrRevisa + cNumero + cItemCNB ) ) //aqui
						//While cNrContra + cNrRevisa  == CNB->CNB_CONTRA + CNB->CNB_REVISA .AND. !Eof()
						While cNrContra + cNrRevisa + cNumero + cItemCNB == CNB->CNB_CONTRA + CNB->CNB_REVISA + CNB->CNB_NUMERO + CNB->CNB_ITEM .AND. !Eof()
					...
					If lSimula
						If aScan(aRelat ,{|x|, x[2] == cNrContra .and. x[3] == cNrRevisa .and. x[9] == nItem .and. x[10] == cProduto }) == 0
							aAdd(aRelat,{xFilial("CND"),cNrContra,cNrRevisa,SA1->A1_COD,SA1->A1_LOJA,SA1->A1_NOME,cNrPlan,cCompet,nItem,cProduto,SB1->B1_DESC,nQuant,nVlrUnt,nQuant*nVlrUnt})
						EndIf
					EndIf
				Next nY
...
Static Function VLDPRDCTR(cNrContra,cNrRevisa,cNrPlan )
	...
	ncontaitem := 1
	While (cAliasCNB)->( !Eof() )
		AADD( aRetorno2, { (cAliasCNB)->CNB_PRODUT, strzero(ncontaitem,3) /*(cAliasCNB)->CNB_ITEM*/, (cAliasCNB)->CNB_VLUNIT, (cAliasCNB)->CNB_XQMIN, (cAliasCNB)->CNB_TABPRC, (cAliasCNB)->CNB_NUMERO, (cAliasCNB)->CNB_ITEM })
		//AADD( aRetorno2, { (cAliasCNB)->CNB_PRODUT, (cAliasCNB)->CNB_ITEM, (cAliasCNB)->CNB_VLUNIT, (cAliasCNB)->CNB_XQMIN, (cAliasCNB)->CNB_TABPRC })
