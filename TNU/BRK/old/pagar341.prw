...
Do Case
    ...
    Case _cTipo == "PP001"	//  Agencia e Conta Corrente Favorecido
       ...
       If sa2->a2_banco = "341"
          ...
          IF _nPosDV == 0 
			 ...
			 _cRetorno += " "+"000000"+strzero(VAL(substr(sa2->a2_numcon,1,06)),06)+" "+strzero(VAL(substr(sa2->a2_numcon,7,01)),01)
		  ELSE
			_nCCItau  := STRZERO(VAL(SUBSTR(sa2->a2_numcon,1,_nPosDV-1)),6) 
			_nDVCCIt  := STRZERO(VAL(SUBSTR(sa2->a2_numcon,_nPosDV+1,1)),1)
       ...
       Else
          ...
          IF _nPosDV == 0
			 ...
			 _cRetorno += " "+strzero(VAL(substr(sa2->a2_numcon,1,10)),12)+" "+substr(sa2->a2_numcon,11,1)
		  ELSE
		    _nCCDif   := STRZERO(VAL(SUBSTR(sa2->a2_numcon,1,_nPosDV-1)),12)
			_nDVDif   := SUBSTR(sa2->a2_numcon,_nPosDV+1,1)
    ...
    Case _cTipo == "PT001"	                           
         ...
         ElseIf (SEA->EA_MODELO == "17")   
            ...
            _cRetorno +=  STRZERO(Val(SE2->E2_E2_YCODREC),4)  
         ...
         ElseIf  SEA->EA_MODELO == "18"   
            ...
            //--- Mensagem ALERTA que está sem periodo de apuração
            If Empty(SE2->VENCREA)  //e2_e_apur
