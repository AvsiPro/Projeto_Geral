...
STATIC function BRWSCTR(cA1CGC)
	...
	if oRet:CMENSAGEMDERETORNO == "OK"
		...
		AADD( aRetorno,{ "NPERIODOFATURAMENTOCOMUMMOTORISTA"       , oRet:NPERIODOFATURAMENTOCOMUMMOTORISTA       , "SV0000000000021" } )
		AADD( aRetorno,{ "NPERIODOFATURAMENTOVEICULO"              , oRet:NPERIODOFATURAMENTOVEICULO              , "SV0000000000021" } )
...
STATIC function VALIDACTR(aMedCTR)
	...
	//  LOCAL cCli       := (cTrbCTR)->CNA_CLIENT
	//  LOCAL cLoja      := (cTrbCTR)->CNA_LOJACL
	LOCAL cTabpr     := ""
	...
	for nX := 1 to LEN( aProdCTR)
		...
		if aMedCTR[6][2] > aMedCTR[7][2]

			AADD( aProduto, { aMedCTR[6][3], aMedCTR[6][2] } )

		else

			AADD( aProduto, { aMedCTR[7][3], aMedCTR[7][2] } )

		Endif



        if aMedCTR[3][2] > aMedCTR[4][2]

			AADD( aProduto, { aMedCTR[3][3], aMedCTR[3][2] } )

		else

			AADD( aProduto, { aMedCTR[4][3], aMedCTR[4][2] } )
		...
		if lRet
			...
			for nY := 1 to LEN(aProduto)
				...
				if lRet
					...
					cTabpr    := CNB->CNB_TABPRC
					cQmin     := CNB->CNB_XQMIN

					cMsg += "Produto: "+ aProduto[nY][01] + " Qtd.: " +  STR(aProduto[nY][02]) +  "Qtd. Min.: " + str(cQmin) +chr(13) + chr(10)
                    ...
                    //Regra de quantidade minima
					//--------------------------

					if cQmin > nQuant

					AADD( aItemCNE[LEN(aItemCNE)], { "CNE_QUANT" , cQmin   , nil } )
                   ...
                   Endif
					...
					AADD( aItemCNE[LEN(aItemCNE)], { "CNE_TABPRC", cTabpr   , nil } )
					...
					//  AADD( aItemCXN[LEN(aItemCXN)], { "CXN_CLIENT" , cCli , nil } )
					//  AADD( aItemCXN[LEN(aItemCXN)], { "CXN_LJCLI" , cLoja , nil } )

				endif

			next nY

			U_zMsgLog(cMsg, 'Quantidade Protheus')

			if lRet		

				//Processa Medicao - CNTA121

				U_GRAVMED(aCabCND, aItemCNE, aItemCXN)
			endif
...
@type function
/*/
User Function GRAVMED(_aCabec, _aItCNE, _aItCXN)
