...
User Function ProcesBol(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo,_lSche, _cDirArqBol, _lMail ,_lGerouPdf )
	...
	If !_lSche
		oObj := MsNewProcess():New({|lEnd| _lGerouPdf := BOLITAU(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo,_lSche, _cDirArqBol,_lMail) },"Processando","Gerando Boletos...",.T.)	//Processamento da geração de boletos
		oObj:Activate()
	Else
...
Static Function BOLITAU(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo,_lSche, _cDirArqBol,_lMail)
	...
	For i := 1 to Len(aVetor)
		...
		If aVetor[i,nPosSel]
			...
			If !DbSeek(aVetor[i,nPosFil] + aVetor[i,nPosSerie] + aVetor[i,nPosNota] + aVetor[i,nPosParc] + aVetor[i,nPosTip])
				...
				Else
					FWAlertHelp( cMen, "Verifique se realmente existe o título" )
				Endif
				    _lRet := .F.
				Exit
			...
			if lBoleto
				...
				If !_lSche
					IF lDebug
						MsgAlert( "Irá gerar o PDF no caminho: "+_cDirArqBol )
					ENDIF					
				...
				oPrint:Say (0046, 0135,"Endereço",oFont06N )
				...
				oPrint:Say (0056,0265,Transform(cCgcSM0,"@R 99.999.999/9999-99"),oFont05) //Cnpj do Beneficiário
				BuzzBox  (0040,0320,0065,0410)	// Box Agencia / Codigo do Cedente
				oPrint:Say (0046, 0321,"Agência/Código do Beneficiário",oFont06N )
				...
				oPrint:Say (0070, 0481,"(  )Não existe nº indicado"	,oFont06N,100)
				oPrint:Say (0080, 0481,"(  )Recusado"               ,oFont06N,100)
				oPrint:Say (0090, 0481,"(  )Não procurado"          ,oFont06N,100)
				oPrint:Say (0100, 0481,"(  )Endereço insuficiente"  ,oFont06N,100)
				...
				oPrint:Say  (0191, 0096,"ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ",oFont06N )
				oPrint:Say  (0201, 0096,"APÓS O VENCIMENTO, SOMENTE NO ITAÚ ",oFont06N )
				...
				oPrint:Say (0211, 0196,"Endereço",oFont06N )
				...
				oPrint:Say (0221,0361,Transform(cCgcSM0,"@R 99.999.999/9999-99"),oFont06) //Cnpj do Beneficiário
				BuzzBox  (0205,0425,0230,0560)	// Agencia 	/ Codigo do Cedente
				oPrint:Say (0211, 0426,"Agência/Código de Beneficiário",oFont06N )
				...
				/*If !Empty(SEE->EE_XJUROS)
					oPrint:Say  (0306,0026,"Juros mora de " + Alltrim(Transform(SEE->EE_XJUROS,"@E 99,999,999.99"))+ " % ao mês ", oFont08,100)
				EndIf
				...
				Endif*/
				//oPrint:Say  (0326,0026,"Sr. Caixa não receber apos vencimento. ", oFont08,100)
				...
				EndIf
				oPrint:Say  (0346,0026,"Protestar apos " + Alltrim(_cProtesto) + " dias úteis do vencimento.", oFont08,100)
				...
				oPrint:Say  (0511, 0096,"ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ",oFont06N )
				oPrint:Say  (0521, 0096,"APÓS O VENCIMENTO, SOMENTE NO ITAÚ ",oFont06N )
				...
				oPrint:Say (0541,0361,Transform(cCgcSM0,"@R 99.999.999/9999-99"),oFont06) //Cnpj do Beneficiário
				BuzzBox  (0525,0425,0550,0560)	// Agencia / Codigo do Cedente
				oPrint:Say (0531, 0426,"Agência/Código do Beneficiário",oFont06N )
				...
				/*If !Empty(SEE->EE_XJUROS)
					oPrint:Say  (0626,0026,"Juros mora de " + Alltrim(Transform(SEE->EE_XJUROS,"@E 99,999,999.99"))+ " % ao mês ", oFont08,100)
				EndIf
				If !Empty(SEE->EE_XMULTA)	
					oPrint:Say  (0636,0026,"Multa por atraso do pagamento de R$ " + Alltrim(Transform(SEE->EE_XMULTA,"@E 99,999,999.99")), oFont08,100)
				Endif*/
				//oPrint:Say  (0646,0026,"Sr. Caixa não receber apos vencimento. ", oFont08,100)
		
				If !Empty(SE1->E1_DECRESC)
					oPrint:Say  (0656,0026,"Conceder Desconto de R$ ..... " + AllTrim(Transform((SE1->E1_DECRESC),"@E 99,999,999.99")), oFont08,100)
				EndIf
				oPrint:Say  (0666,0026,"Protestar apos " + Alltrim(_cProtesto) + " dias úteis do vencimento.", oFont08,100)
				...
				IF lDebug
				   if file(_cDirArqBol + _cArquivo + ".rel")
				      ...
				      cMen := "Arquivo: "+_cDirArqBol + _cArquivo + ".rel - Não Encontrado!!!"
				   endif 
					MsgAlert( cMen )
				ENDIF				
				...
				if lChk2
				    IF lDebug
					   MsgAlert( "Habilitado para gerar uma cópia na maquina local") 
					endif 
					if File(_cDirArqBol + _cArquivo + ".pdf")
						__CopyFile(_cDirArqBol + _cArquivo + ".pdf", cDirLocal+_cArquivo + ".pdf")
						if lDebug
						   MsgAlert( "Cópia realizada na maquina local. Caminho: "+cDirLocal) 
						endif 
					endif 
				endif 
	...
	Next
	/*  Conforme orientação do Claudio, não deverá excluir o arquivo da pasta
	If !_lRet
		For _nx := 1 to Len( _aFiles )
			fErase( _aFiles[_nx] )
		Next
...
description
Rotina para Atualização do Nosso Numero, Codigo Barra e Linha Digitavel
@type function
@version  1.0
@author Valdemir Rabelo
@since 16/01/2022
...
Static Function CodBco341(_cNumeIni,cInull,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo,_lSche)
	...
	If Empty( _cNossoNum )
		cMen := "Nosso número não preenchido para o Titulo " + Alltrim( _cTitulo ) + ", verifique se a tabela de parametros para o banco informado esta preenchida (SEE)."
		If _lSche
			BrkClassGen():MsgConMon( cMen, .T. )
		Else
			FWAlertInfo( cMen, "Atenção!" )
		Endif
	...
	_cDigBar	:= ""
	_cNumBar	:= _fNumBar(_cBanco,_cAgencia,_cConta,_cNossoNum,@_cDigBar,_cNossoDig)   // Montagem do Código de Barras.
...
/*/{@Protheus.doc} _fNumBar
	description
	Rotina para Montagem do Código de Barras.
	@type function
	@version  1.0
	@author Valdemir Rabelo
	@since 16/01/2022
...
Static Function Nosso341(_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo,_lSche)
	...
	If Empty(SE1->E1_NUMBCO)
		...
		Else
			cMen := "Não existe configuração para o banco informado SEE do Titulo " + Alltrim(_cTitulo) + ", favor preencher."
			If _lSche
				BrkClassGen():MsgConMon( cMen, .T. )
			Else
				FWAlertInfo( cMen, "Atenção!" )
