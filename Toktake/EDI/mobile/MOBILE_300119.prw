...
dbSelectArea("SZS")
dbSetOrder(2)
If msSeek( xFilial("SZS") +AvKey( aDados[3],"ZS_CODTEC" ) )    
...
If cCont == "S"  
	...
	If MSSeek( xFilial("SN1") +AvKey(aDados[5],"N1_CHAPA") )
	...
	/*
	aaNotif[#'vld'][1][#'p1'] := aContAnt[5]
	aaNotif[#'vld'][1][#'p2'] := aContAnt[6]
	aaNotif[#'vld'][1][#'p3'] := aContAnt[7]
	aaNotif[#'vld'][1][#'p4'] := aContAnt[8]
	aaNotif[#'vld'][1][#'p5'] := aContAnt[9]
	aaNotif[#'vld'][1][#'p6'] := aContAnt[10]
	aaNotif[#'vld'][1][#'p7'] := aContAnt[11]
	aaNotif[#'vld'][1][#'p8'] := aContAnt[12]
	aaNotif[#'vld'][1][#'p9'] := aContAnt[13]
	aaNotif[#'vld'][1][#'p10'] := aContAnt[14]
	aaNotif[#'vld'][1][#'p11'] := aContAnt[15]
	aaNotif[#'vld'][1][#'p12'] := aContAnt[16]
	aaNotif[#'vld'][1][#'p13'] := aContAnt[17]
	aaNotif[#'vld'][1][#'p14'] := aContAnt[18]
	aaNotif[#'vld'][1][#'p15'] := aContAnt[19]
	aaNotif[#'vld'][1][#'p16'] := aContAnt[20]
	*/
...
If cEntrega == "S"
	aaNotif[#'fentrega'] := "S"
	//aConf := getConfs()
	//aConf := {{"800429-FELIPE BORGES MEDINA LOPES"},{"400262-JACKSON EZIDIO DE DEUS"}}
	/*
	If Empty(aConf)
		MsgInfo("Nใo foram atendentes!")
		cNumOS := ""
		Return cNumOS
	EndIf
	
	aaNotif[#'conferentes'] := Array(Len(aConf))
	For nI := 1 To Len(aConf)
		aaNotif[#'conferentes'][nI] := Array(#)
		aaNotif[#'conferentes'][nI][#'cnf'] := aConf[nI][1]
	Next nI	
	*/
EndIf
...
dbSetOrder(1)
cNumOLD := cNumOS
While MsSeek( xFilial("SZG") +AvKey(cNumOs,"ZG_NUMOS") )
	cNumOS := RetNum() 
End
...
If !Empty(cRegID) .And. !Empty(cNPush)
	//If ALLTRIM(GetEnvServer()) $ "JVVXC9#UPD#CNP#SCHEDULE"
...
If !Empty(cNPush2)
	//MemoWrite(cDirSend +cNumOS +"_json_os.json" ,cNPush2)
EndIf
...
dbSelectArea("AA1")
dbSetOrder(1)
If msSeek( xFilial("AA1") +AvKey(cAtendente,"AA1_CODTEC") )
...
If MsSeek( xFilial("AA1") +AvKey(cAtendente,"AA1_CODTEC") )
	If AllTrim(AA1->AA1_XARMOV) == "S"
		...
		If MsSeek( xFilial("AA1") +AvKey(cAtendLog,"AA1_CODTEC") )
