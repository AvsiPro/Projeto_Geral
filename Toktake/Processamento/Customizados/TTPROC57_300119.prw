...
±±∫Programa  ≥TTPROC57    ∫Autor  ≥Jackson E. de Deus∫ Data ≥  30/06/17   ∫±±
±±∫							Xcode Mobile								  ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Painel de movimentos de produtos - Acompanhamento/Fecham.  ∫±±
±±∫          ≥                                                            ∫±±
...
If Empty(cRota)
	...
	If !ParamBox(aPergs ,"Invent·rio",@aRet)
		Return
	Else
		cRota := aRet[1]
		If Empty(cRota)
			MsgAlert("Informe uma Rota/PA v·lida!")
			Return
		EndIf                                
	EndIf
EndIf                
...
// nf dev 
ChkNfDv( _cRota,dIniCClo,cHrIniCC,dDatabase, @cNfDev,@cSeriDev )
If !Empty(cNfDev) 
	_lNFDev := .T.
...
oTelaF := MSDialog():New( aSize[7],aSize[1],aSize[6],aSize[5],"Fechamento - "+cRota +" " +cDescZZ1,,,.F.,,,,,,.T.,,,.T. )
	...
	oLayer:addCollumn('COLUNA1_1',100,.F.,"LINHA1")
  	...
  	oLayer:addWindow("COLUNA1_1","WIN_2","An·lise dos produtos - " +cDescZZ1,100,.T.,.F.,{||  },"LINHA1")
	oPnl1 := oLayer:GetWinPanel("COLUNA1_1","WIN_2","LINHA1")

	oLista1 := TCBrowse():New(0,0,0,0,, {"","Produto","DescriÁ„o","Estoque ant.","Carga recebida","Abastecido","Retirado","Saldo","Estoque contado",;
									"DevoluÁ„o Apontada","Avarias de Maq. a Dev.","Contagem avarias","Div. avarias","Div. totais","Desconto"},;
							{20,50,100,50,50,50,50,50,50,50,60,50,50,50,50},;
	...
	//oSNf := TSay():New( 12,05,{ || "NF DevoluÁ„o: " +cNfDev +"/" +cSeriDev },oPanel3,"",,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,100,12,,,,,,.F. )	
	...
	MENUITEM "Estorno Movimentos"		ACTION ( IIF( VldPerm(8),U_TTESTMOV( cRota,_dIniCiclo,dDatabase,_cHrIniCC),/**/ ) )	
	...
	oBt2 := TButton():New( 05, nWdtRdp-400,"1.OS Invent·rio",oPanel3,{ || gerarInv(_dIniCiclo,dDAtabase,_cHrIniCC) },040,015,,,,.T.,,"Gerar a conferÍncia dos produtos",,,,.F. )
	...
	oBtInv:SetCss( "QPushButton{ background-color: #B9B9B9; border: 1px solid gray; border-radius: 0px; background-repeat: none; margin: 1px }" )			
   	...
   	oBtCnc := TButton():New( 05, nWdtRdp-320,"3.Cancelar Inv.",oPanel3,{ || CancInv() },040,015,,,,.T.,,"Cancelar a OS de invent·rio",,,,.F. )
	...
	oBtNF := TButton():New( 05, nWdtRdp-280,"4.DevoluÁ„o",oPanel3,{ || NFDEV() },040,015,,,,.T.,,"Gerar devoluÁ„o de produtos",,,,.F. )
    ...
    oBt3 := TButton():New( 05, nWdtRdp-240,"5.RelatÛrio",oPanel3,{ || U_TTESTR15( cRota,_dIniCiclo,dDatabase,_cHrIniCC,_aLista1, _aNF,_cNfDev, _cSerDv ) },040,015,,,,.T.,,"RelatÛrio de movimentos do periodo",,,,.F. )
    ...
    oBtDiv := TButton():New( 05, nWdtRdp-200,"6.Ajuste Div.",oPanel3,{ || AjusteDiv() },040,015,,,,.T.,,"Ajusta divergÍncias",,,,.F. )
	oBtDiv:SetCss( "QPushButton{ background-color: #B9B9B9; border: 1px solid gray; border-radius: 0px; background-repeat: none; margin: 1px }" )	
   	...
   	oBt4 := TButton():New( 05, nWdtRdp-160,"7.Pedidos",oPanel3,{ || PedDesc()  },040,015,,,,.T.,,"Gerar pedidos de descarte e perda",,,,.F. )
	...
	oBt5 := TButton():New( 05, nWdtRdp-120,"8.Encerrar",oPanel3,{ || lFim := FimC2(lPend) ,IIF(lFim,oTelaF:End(),)  },040,015,,,,.T.,,"Encerrar a prestaÁ„o de contas",,,,.F. )
...
If !EOF()
	...
	/*If lOpen
		Aviso("TTPROC57","Essa Rota/PA est· em fechamento, iniciado no dia " +dtoc(stod(TRBZ->ZG_DATAINI)) +CRLF +"Entre o sistema com a data mencionada e encerre o fechamento.",{"Ok"})
		oTelaF:End()
	EndIf*/
EndIf 
...
Local cPerm8 := "JDEUS#JNASCIMENTO#RJESUS"
...
For nI := 1 To Len( _aLista1 )
	...
	//nDivTot := ( nConfBom - nDevOs - nQAtu )  // ALTERADO POIS APOS GERAR A DEVOLUCAO, O SALDO BAIXA E GERA DIVERGENCIA POSITIVA NA COLUNA DIVERGENCIAS TOTAIS
	...
	/*	
	13.	DivergÍncias Totais: (Estoque Contado Bom - Saldo) + se DivergÍncia de Avarias > 0, soma, sen„o n„o soma.
...
If !VldPerm(7)
	MsgAlert("VocÍ n„o tem permiss„o para acessar esse recurso!")
	Return lFim
EndIf


If Empty(_cOsInvent)
	MsgAlert("… necess·rio contar o estoque atravÈs do invent·rio!")
	Return lFim
Else
	dbSelectArea("SZC")
	dbSetOrder(3)
	If !MsSeek( xFilial("SZC") +AvKey(_cOsInvent,"ZC_NUMOS") )	
		MsgAlert("Necess·rio processar as contagens.")
		Return lFim
	EndIf
EndIf
...
If MsSeek( xFilial("SZG") +AvKey( _cOsInvent,"ZG_NUMOS" ) )
	If dDatabase > SZG->ZG_DATAFIM
		MsgAlert("AtenÁ„o, o invent·rio est· registrado com data anterior ( " +dtoc(SZG->ZG_DATAFIM) +"). Cancele o invent·rio ou entre com a data correta para o encerramento.")
		Return lFim
	EndIf
EndIf


If !MsgYesNo("Confirma a finalizaÁ„o da prestaÁ„o de contas? Essa operaÁ„o n„o poder· ser desfeita.")
	Return lFim
EndIf
...
If Empty(aProd)
	MsgInfo("N„o houve abastecimento.")
	Return
EndIf
...
oWnd := MSDialog():New( aSize[7],aSize[1],aSize[6],aSize[5],"",,,.F.,,CLR_WHITE,,,,.T.,,,.T. )		
	...
	oLstDet:AddColumn(TCColumn():New('DescriÁ„o'		,{|| aProd[oLstDet:nAt][2] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,)) 
	...
	oLstDet:AddColumn(TCColumn():New('PatrimÙnio'		,{|| aProd[oLstDet:nAt][7] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,))
...
Private cCadastro := "ManutenÁ„o de Ordens de ServiÁo"
...
If _lPedOk
	MsgInfo( "Os pedidos j· foram gerados." )
	Return
EndIf

If Empty(_cOsInvent)
	MsgAlert("Antes da geraÁ„o dos pedidos, È preciso gerar a OS de invent·rio e processar as contagens.")
	Return
EndIf
...
If !MsSeek( xFilial("SZC") +AvKey(_cOsInvent,"ZC_NUMOS") )	
	//If AllTrim( SZC->ZC_PROC ) <> "S"
		MsgAlert("Antes da geraÁ„o dos pedidos, È preciso processar as contagens.")
		Return
	//EndIf	
EndIf
//TabDocs()
...
If nPend > 0
	...
	Next nI
	Aviso("TTPROC57","Essa Rota/PA ainda possui OS pendente de finalizaÁ„o:" +CRLF +cMsg,{"Ok"})
	Return
EndIf
If nProc > 0
	For nI := 1 To Len(aOS)
...
Else
	...
	cMsg := ""
	If Empty(cCodTec)
		cMsg := "PA sem atendente padr„o configurado." +CRLF
		cMsg += "Escolha um atendente para receber a Ordem de ServiÁo de invent·rio."
	...
	Else
		...
		cMsg := "Atendente padr„o: " +cNome
		cMsg += "Deseja alterar o atendente?"
		
		nOpc := Aviso("TTPROC57",cMsg,{"Sim","N„o"})
 		If nOpc == 1
			If ( ConPad1(,,,"AA1",,,.F.) )
				cCodTec := AA1->AA1_CODTEC
			Else      
				MsgInfo("GeraÁ„o de OS cancelada..")
				Return	
			EndIf
		EndIf 
...
If Empty(cCodTec)
	MsgAlert("Escolha um atendente v·lido para realizar a contagem!")
	Return
EndIf
...
MsAguarde({ || cOsInvent := U_TTPROC42( _cRota,cCliFor,cLoja,cCodTec,dInicio,dFim,_cHrIniCC ) },"Gerando os de invent·rio..." )
...
If !Empty( cOsInvent )
	...
	MsgInfo( "OS de invent·rio gerada: " +cOsInvent )	
EndIf
...
// verifica status
If Empty( _cOSInvent )
	MsgAlert("Primeiro inicie a prestaÁ„o de contas!")
	Return
EndIf
...
// verifica se ja gerou a devolucao
/*dbSelectArea("SZ7")
For nI := 1 To Len(_aDocIt)
	dbGoTo(_aDocIt[nI][12])
	If Recno() == _aDocIt[nI][12]
	
		cNumNF := SZ7->Z7_DOCRET
		cSerie := SZ7->Z7_SERIRET
		Exit
...
If !Empty(_cNfDev) .And. !Empty(_cSerDv)
	MsgInfo( "A nota de devoluÁ„o j· foi criada!" +CRLF +_cNfDev + "/" +_cSerDv )
Else
	dbSelectArea("SZG")
	dbSetOrder(1)
	If MsSeek( xFilial("SZG") +AvKey( _cOsInvent,"ZG_NUMOS" ) )
		If AllTrim(SZG->ZG_STATUS) <> "FIOK"
			MsgInfo("A conferÍncia ainda n„o foi finalizada.")
		Else
	...
	If Empty( aItens )
		MsgAlert("Processe as contagens do invent·rio.")
		Return
	EndIf
EndIf	
...
If !lDevolver
	MsgInfo( "N„o h· mercadoria apontada para devoluÁ„o." )
	Return
EndIf
...
For nI := 1  To Len(_aLista1)
	...
	fNfSeek( cProduto, nQtdDev, @axLst )
...
If !Empty(axLst)
	...
	cMsgNF += " Usu·rio: " +cUserName
	           
	// ajustar as quantidades das notas
	If FindFunction( "U_TTOPER19" )
		U_TTOPER19( _cRota, @axLst )
	EndIf
	 

	MsAguarde( { || cNumNF := U_TTMT140(axLst,cMsgNF) },"Gerando nota de devoluÁ„o.." )
	...
	If !Empty(cNumNF)
		...
		MsgInfo( "Nota de devoluÁ„o gerada: " +cNumNF )
	EndIf	
EndIf
...
/*
Procura os produtos nas notas
*/
Static Function fNfSeek( cProduto, nFalta, axLst )
...
// PROCURAR NOTAS
cQuery := "SELECT TOP 100 D2_DOC,D2_SERIE, D2_ITEM ,D2_LOCAL, D2_PRCVEN, D2_QUANT,D2_TES,'D1TOTAL' = ( "
...
cQuery += " AND D2_QUANT >= '"+cvaltochar(nFalta)+"' " 
cQuery += " AND F2_XCODPA = '"+_cRota+"' " 
cQuery += " AND SF2.D_E_L_E_T_ = '' " 
cQuery += " AND F2_EMISSAO < '"+dtos(_dIniCiclo)+"' " 
cQuery += " AND F2_XFINAL = '4' "				
...
//	If AScan( _aDocIt, { |x| AllTrim(x[1]) == AllTrim(TRBD2->D2_DOC) } ) == 0		
		If ( TRBD2->D2_QUANT - TRBD2->D1TOTAL ) >= nFalta
		
			
			AADD( axLst, { cProduto,;				// produto
							nFalta,;				// qtd
							TRBD2->D2_PRCVEN,;		// preco
							TRBD2->D2_DOC,;			// nota
							TRBD2->D2_SERIE,;		// serie
							TRBD2->D2_ITEM,;		// item
							TRBD2->D2_TES,;			// tes
							TRBD2->D2_LOCAL  } )	// local		
...
If !MSSeek( xFilial("SZC") +AvKey( cOSConf,"ZC_NUMOS" ) )
	//MsgAlert( "Ordem de serviÁo de invent·rio pode n„o ter sido processada. Verifique." )
	Return
EndIf
...
If Empty( _cOsInvent )
	MsgInfo("N„o gerou a OS de invent·rio.")
	Return
EndIf                                     

If !MsgYesNo("Deseja cancelar o invent·rio?")
	Return
EndIf
...
If MSSeek( xFilial("SZG") +AvKey( _cOsInvent,"ZG_NUMOS") )
	If AllTrim(SZG->ZG_STATUS) <> "FIOK"
		STATICCALL( TTPROC30, ProcCanc, _cOsInvent,_cOsInvent,SZG->ZG_ROTA,SZG->ZG_ROTA,SZG->ZG_DATAINI,SZG->ZG_DATAINI,1,"CANCELADO VIA CENTRAL" )
	ElseIf AllTrim(SZG->ZG_STATUS) == "FIOK"
		RecLock("SZG",.F.)
		SZG->ZG_STATUS := "COPE"
...
oWnd := MSDialog():New( aSize[7],aSize[1],aSize[6],aSize[5],"Notas fiscais",,,.F.,,CLR_WHITE,,,,.T.,,,.T. )		
	...
	oLstDet:AddColumn(TCColumn():New('SÈrie'		,{|| aNotas[oLstDet:nAt][3] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,)) 
	oLstDet:AddColumn(TCColumn():New('Emiss„o'		,{|| aNotas[oLstDet:nAt][4] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,)) 
	oLstDet:AddColumn(TCColumn():New('SaÌda'		,{|| aNotas[oLstDet:nAt][5] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,))
...
If Empty( _cOsInvent )
	MsgAlert("A Ordem de ServiÁo ainda n„o foi gerada.")
	Return
EndIf
...
If MsSeek( xFilial("SZG") +AvKey(_cOsInvent,"ZG_NUMOS") )
	...
	If AllTrim(SZG->ZG_STATUS) == "FIOK"
		If SZG->ZG_DATAFIM > SZG->ZG_DATAINI
			MsgAlert("O invent·rio foi finalizado com data posterior ao agendamento. Cancele e inicie outro invent·rio.")
			Return
		EndIf
		
		aDados := U_MXML000(SZG->ZG_RESPOST)
	Else
		MsgInfo("Invent·rio n„o finalizado.")
		Return	
	EndIf
...
If MSSeek( xFilial("SZC") +AvKey( _cOsInvent ,"ZC_NUMOS") )
	MsgInfo("Invent·rio j· processado.")
	Return
EndIf
...
oWnd := MSDialog():New( aSize[7],aSize[1],aSize[6],aSize[5],"Pedidos gerados",,,.F.,,CLR_WHITE,,,,.T.,,,.T. )		
	...
	oLstDet:AddColumn(TCColumn():New('Emiss„o'		,{|| aPedido[oLstDet:nAt][2] },"@!",,,'LEFT',,.F.,.F.,,,,.F.,)) 
...
Static Function ChkNfDv( cArmazem,dInicio,cHrIniCC,dFim,cNfDev,cSeriDev )
...
cQuery += " WHERE Z5_FILIAL = '"+xFilial("SZ5")+"' AND Z5_NF = '"+cNF+"' D_E_L_E_T_ = '' "
...
MsgInfo( "… preciso imprimir o relatÛrio antes do ajuste de divergÍncias." )

nOpcao := Aviso( "Ajustes","Escolha a opÁ„o: Imprimir o relatÛrio ou Ajustas as divergÍncias",{"Imprimir","Ajustar"} )
...
/*
Somente dos valores negativos: Emitir pedido de Perda, para PA. Faturar a perda(baixa de A, P ou R se Rota).Obs na Nota e ajuste:"Perda de PA - Fechamento de Rota: OS de Inventario; PA/Rota; Nome do TÈcnico; Nome do Usu·rio que gerou" - Gera desconto para funcion·rio.
Para os valores positivos: Gerar movimentos de entrada na PA, em Pe no A, e R se Rota. (se n„o entrar, n„o conseguir· faturar a devoluÁ„o) - Tem que registrar o tipo de movimento de entrada - Inventario de Rota/PA.
*/
