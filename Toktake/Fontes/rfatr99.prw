...
*/
User Function RFATR99(cNotIni, cNotFim, cSerie, nNotTip, lBoleto)
...
//aAdd(aRegs,{cPerg,"05"," Boleto Banco",	"","","mv_ch5","N",01,0,0,"C","","MV_PAR05","Sim",		"","",		"","","Nao",	"",		"",		"","","","","","","","","","","","","","","","",		"",		"","","",""})
...
Private c_MensItem	:= ""							// Mensagem no Item
Private n_Descont	:= 0



oPrint := TMSPrinter():New("Espelho de Notas")  
oPrint:SetLandscape()  
oPrint:SetPaperSize(9)
oPrint:Setup()     
oPrint:StartPage()                  

oFont1 := TFont():New('Courier new',,-7,.T.,.T.)
oFont2 := TFont():New('Courier new',,-15,.T.,.T.)
oFont3 := TFont():New('Courier new',,-15,.T.,.F.)
			...
			For nLoop := 1 To nQdeLnh
				...
				Else
					...
					Else
						...
						If( SD2->D2_PRUNIT>0 .And. SC5->C5_XFINAL=="2" .And.(SD2->D2_PRUNIT-SD2->D2_PRCVEN)>0 )
							c_VlrUnit 	:= "SD2->D2_PRUNIT"
							c_VltTot	:= "SD2->D2_TOTAL+SD2->D2_DESCON"
							c_MensItem	:= "DESCONTO TOTAL DE: R$"+ Transform(SF2->F2_DESCONT,"@E 999,999.99")
							n_Descont	:= SF2->F2_DESCONT
						Else
							c_VlrUnit := "SD2->D2_PRCVEN"
							c_VltTot	:= "SD2->D2_TOTAL"							
						EndIf
						...
						aAdd( aItens, {	SD2->D2_COD,;
										...
										SD2->D2_UM,;
										SD2->D2_QUANT,;
										If(Empty(SD2->D2_DESCZFR),&(c_VlrUnit),Round((&(c_VltTot)+SD2->D2_DESCZFR)/SD2->D2_QUANT,2) ),;
										If(Empty(SD2->D2_DESCZFR),&(c_VltTot),Round(&(c_VltTot)+SD2->D2_DESCZFR,2)),;
										SD2->D2_PICM,;
...
Else	// Compras - Entradas
	...
	While !Eof() .And. SF1->F1_FILIAL == xFilial("SF1") .And. SF1->F1_DOC <= mv_par02 .And. lContinua 
		...
		*/
		...
		//³Calcula a quantidade de formularios necessarios³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		nFormul := Int( IIF(Empty(c_MensItem),Len(aItens),Len(aItens)+1)/nTamDet + .999999999)
...
//@ 000, 000 PSAY Chr(18)					  // Descompressao de caracteres // comentado jackson
...
If mv_par04 == 2
	...
	oPrint:Say(Li,900,"X",oFont1,20,CLR_BLACK ) 
	oPrint:Say(Li,1400,aCab[01] + If(nFormul > 1," / "+AllTrim(Str(nForAtu,6)),""),oFont1,300,CLR_BLACK ) 	
Else
	//@ Li,090 PSAY "X"
	//@ Li,108 PSAY aCab[01] + If(nFormul > 1," / "+AllTrim(Str(nForAtu,6)),"")//Numero
	
	oPrint:Say(Li,1000,"X",ofont1,20,CLR_BLACK ) 
	oPrint:Say(Li,1500,aCab[01] + If(nFormul > 1," / "+AllTrim(Str(nForAtu,6)),""),ofont1,300,CLR_BLACK ) 
EndIf
Li+= 200
//@ Li,004 PSAY Left(cNatOpe, 40)
//@ Li,043 PSAY Left(cCFOOpe, 14)

oPrint:Say(Li,020,Left(cNatOpe, 40),ofont1,400,CLR_BLACK ) 
oPrint:Say(Li,500,Left(cCFOOpe, 14),ofont1,100,CLR_BLACK ) 

Li+= 100
//@ Li,004 PSAY Alltrim(aCab[7])+" - "+aCab[5]+"-"+aCab[6]//Nome do Cliente + Codigo + Loja
oPrint:Say(Li,020,Alltrim(aCab[7])+" - "+aCab[5]+"-"+aCab[6],ofont1,500,CLR_BLACK ) 

If Len(Alltrim(aCab[15])) > 11		//CGC
	//@ Li,080 PSAY Transform(aCab[15], "@R 99.999.999/9999-99")
	oPrint:Say(Li,1400,Transform(aCab[15], "@R 99.999.999/9999-99"),ofont1,300,CLR_BLACK ) 	
Else
	//@ Li,080 PSAY Transform(aCab[15], "@R 999.999.999-99")
	oPrint:Say(Li,1400,(aCab[15], "@R 999.999.999-99"),ofont1,300,CLR_BLACK ) 	
EndIf

//@ Li,111 PSAY aCab[4]//Data de Emissao
oPrint:Say(Li,1700,dtoc(aCab[4]),ofont1,300,CLR_BLACK ) 	
...
oPrint:Say(Li,020,aCab[8],ofont1,500,CLR_BLACK ) 	
oPrint:Say(Li,700,aCab[9],ofont1,300,CLR_BLACK ) 	
oPrint:Say(Li,1100,Transform(aCab[10], "@R 99999-999"),ofont1,300,CLR_BLACK ) 	


Li+= 100
//@ Li,004 PSAY aCab[11]//Município
// x @ Li,050 PSAY aCab[13]+" "+aCab[14] // Incluido em 26/05/06
...
oPrint:Say(Li,020,aCab[11],ofont1,300,CLR_BLACK ) 
oPrint:Say(Li,600,If(!Empty(Alltrim(aCab[13])), "("+Alltrim(aCab[19])+")"+Transform(AllTrim(aCab[13]), "@R 9999-9999")," "),ofont1,200,CLR_BLACK ) 
oPrint:Say(Li,800,aCab[12],ofont1,20,CLR_BLACK ) 
oPrint:Say(Li,1000,Transform(aCab[16], "@R 999.999.999.999"),ofont1,300,CLR_BLACK ) 
...
if empty(cMensPA)
	If Len(aDuplic) >= 1
		...
		oPrint:Say(Li,020,cDuplic+" - "+cTpPag,ofont1,300,CLR_BLACK ) 
	
	EndIf
else
		//@ Li,004 PSAY cMensPA+" - "+cTpPag
		oPrint:Say(Li,020,cMensPA+" - "+cTpPag,ofont1,300,CLR_BLACK ) 
...
If nVlZfr > 0
	...
	oPrint:Say(Li,020,"Desconto de ICMS 7% R$",ofont1,300,CLR_BLACK ) 
	
	//@ Li,026 PSAY Round((nVlZfr*0.430769),2) Picture "@E 999,999.99" //Alterado por Cadu em 4/2/10, segundo calculos analisados pela Patricia
	//@ Li,026 PSAY Round((nVlZfr*0.445344),2) Picture "@E 999,999.99"
	oPrint:Say(Li,200, Transform( Round((nVlZfr*0.445344),2),"@E 999,999.99" ),ofont1,300,CLR_BLACK ) 
	
	//@ Li,038 PSAY "Desconto de PIS/COFINS 9,25% R$"
	oPrint:Say(Li,400, "Desconto de PIS/COFINS 9,25% R$",ofont1,300,CLR_BLACK ) 
	
	//@ Li,070 PSAY Round((nVlZfr*0.56923),2) Picture "@E 999,999.99" //Alterado por Cadu em 4/2/10, segundo calculos analisados pela Patricia
	//@ Li,070 PSAY Round((nVlZfr*0.58849),2) Picture "@E 999,999.99"
	oPrint:Say(Li,600, Transform( Round((nVlZfr*0.58849),2), "@E 999,999.99"),ofont1,300,CLR_BLACK ) 	
...
For nLoop := 1 To Len(aDadAdc)
	...
	oPrint:Say(Li,020, aDadAdc[nLoop,2],ofont1,300,CLR_BLACK ) 	
	Li+=20
...
For nLoop2 := 1 To nLinNbm
	...
	oPrint:Say(Li,020, cMemoNbm,ofont1,1500,CLR_BLACK ) 	
...
If nFormul > 1 .And. nForAtu <> nFormul
	...
	oPrint:Say(Li,100, "****,***,***.**",ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,500, "****,***,***.**",ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,900, "****,***,***.**",ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,1300, "****,***,***.**",ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,1700, "****,***,***.**",ofont1,300,CLR_BLACK ) 	
...
Else
	...
	oPrint:Say(Li,100, Transform(aRodape[12],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,500, Transform(aRodape[13],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,900, Transform(aRodape[16],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,1700, Transform(aRodape[17],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 	
	
	IF nVlZfr > 0
	  //@ Li,108 PSAY (aRodape[02]-nTotSer) + nVlZfr +n_Descont Picture "@E 9999,999,999.99" //Valor  da Mercadoria
		oPrint:Say(Li,2100, Transform((aRodape[02]-nTotSer) + nVlZfr +n_Descont,"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 		
	ELSE
	   //@ Li,108 PSAY aRodape[02]-nTotSer+n_Descont		Picture "@E 9999,999,999.99" //Valor  da Mercadoria
		oPrint:Say(Li,2100, Transform(aRodape[02]-nTotSer+n_Descont,"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 	   
	ENDIF   
EndIf
Li += 20
...
Else
	...
	oPrint:Say(Li,100, Transform(aRodape[04],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,500, Transform(aRodape[08],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,900, Transform(aRodape[09],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,1300, Transform(aRodape[15],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,1700, Transform(aRodape[01],"@E 9999,999,999.99"),ofont1,300,CLR_BLACK ) 		
EndIf
Li += 30
...
oPrint:Say(Li,040, aTransp[03],ofont1,400,CLR_BLACK ) 	
oPrint:Say(Li,500, aTransp[11],ofont1,050,CLR_BLACK ) 	
oPrint:Say(Li,800, aTransp[13],ofont1,060,CLR_BLACK ) 	
oPrint:Say(Li,1200, aTransp[12],ofont1,050,CLR_BLACK ) 	

If !Empty(aTransp[09])//CGC
	If Len(Alltrim(aTransp[09])) >= 14
		//@ Li, 104 PSAY aTransp[09] Picture"@R 99.999.999/9999-99"
		oPrint:Say(Li,1600, Transform(aTransp[09],"@R 99.999.999/9999-99"),ofont1,300,CLR_BLACK ) 
	Else
		//@ Li, 104 PSAY aTransp[09] Picture"@R 999.999.999-99"
		oPrint:Say(Li,1600, Transform(aTransp[09],"@R 999.999.999-99"),ofont1,300,CLR_BLACK ) 			
	Endif		
Endif
//EndIf
Li += 50
...
//If nFormul = 1 .Or. nForAtu == nFormul
	...
	oPrint:Say(Li,040, aTransp[04],ofont1,400,CLR_BLACK ) 
	oPrint:Say(Li,500, aTransp[06],ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,900, aTransp[07],ofont1,300,CLR_BLACK ) 
	oPrint:Say(Li,1200, aTransp[10],ofont1,300,CLR_BLACK ) 	
//EndIf  
...
If (nFormul = 1 .Or. nForAtu == nFormul)
	...
	//x@ Li,095 PSAY aTransp[17]											//Número dos dos Volumes
	      
	oPrint:Say(Li,100, Transform(aTransp[14],"@E 999,999.99"),ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,500, aTransp[15],ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,900, aTransp[16],ofont1,300,CLR_BLACK ) 	
	...
	oPrint:Say(Li,1300, aItens[01,15],ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,1700, Transform(aTransp[18],"@E 999,999.99"),ofont1,300,CLR_BLACK ) 	
	oPrint:Say(Li,2100, Transform(aTransp[19],"@E 999,999.99"),ofont1,300,CLR_BLACK ) 		
EndIf
...
For nLoop := 1 To nQdeLnh
	...
	//@ Li,004 PSAY cMemo
	oPrint:Say(Li,020, cMemo,ofont1,300,CLR_BLACK ) 
...
oPrint:Say(Li,1400, aCab[01],ofont1,300,CLR_BLACK ) 
...
For nLoop1 := (nForAtu-1)*nTamDet+1 To Min(Len(aItens),nForAtu*nTamDet)
	...
	//x@ Li,004 PSAY Left(aItens[nLoop1,01],14)			// Codigo Alterado Ramon 15/01/2010
	//@ Li,004 PSAY Left(aItens[nLoop1,01],7)			// Codigo
	//@ Li,016 PSAY aItens[nLoop1,02]						// Descricao
	oPrint:Say(Li,020,  Left(aItens[nLoop1,01],7),ofont1,100,CLR_BLACK ) 
	oPrint:Say(Li,250, aItens[nLoop1,02],ofont1,400,CLR_BLACK ) 
	...
	If !Empty(aItens[nLoop1,01])
		...
		oPrint:Say(Li,900, aItens[nLoop1,03],ofont1,20,CLR_BLACK ) 
		oPrint:Say(Li,950, aItens[nLoop1,04],ofont1,20,CLR_BLACK ) 
		oPrint:Say(Li,1050, aItens[nLoop1,05],ofont1,20,CLR_BLACK ) 
		oPrint:Say(Li,1150, Transform( aItens[nLoop1,06], "@E 999,999.99" ),ofont1,150,CLR_BLACK ) 
		oPrint:Say(Li,1300, Transform( aItens[nLoop1,07], "@E 999,999.9999" ),ofont1,150,CLR_BLACK ) 		
		oPrint:Say(Li,1450, Transform( aItens[nLoop1,08], "@E 999,999.99" ),ofont1,150,CLR_BLACK ) 
		oPrint:Say(Li,1700, Transform( aItens[nLoop1,09], "@E 99" ) ,ofont1,50,CLR_BLACK ) 
		oPrint:Say(Li,1800, Transform( aItens[nLoop1,10], "@E 99.9" ),ofont1,100,CLR_BLACK ) 
		oPrint:Say(Li,1900, Transform( aItens[nLoop1,11], "@E 999,999.99" ),ofont1,200,CLR_BLACK ) 
	EndIf
...
Li+= 100
If( !Empty(c_MensItem) )
	//@ Li,004 PSAY c_MensItem
	oPrint:Say(Li,020, c_MensItem,ofont1,1500,CLR_BLACK ) 	
EndIf
