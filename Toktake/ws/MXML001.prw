...
CONOUT("#MXML001 -> INICIO DE PROCESSAMENTO: "+DTOC(dFim) +" " +cHrIni )
...
CONOUT("#MXML001 -> FIM DE PROCESSAMENTO: "+DTOC(dFim) +" " +cHrFim + " - TEMPO TOTAL: " +cTempo )
...
cQuery += "WHERE ZG_FORM IN ('04','08','13') AND ZG_STATUS = 'FIOK' AND ZG_DATAFIM >= '"+DTOS(dFim)+"' AND ZG_PROC = '' "
...
CONOUT("#MXML001 -> TOTAL DE OS PARA PROCESSAMENTO: " +CVALTOCHAR(Len(aOSPend)) )
...
Else
	If lNovo
		...
		Else
			Conout("#MXML001 -> XML DE RESPOSTAS INVALIDO")
			Return		
		EndIf
	Else 
...
cHrIni := Time()
CONOUT( "#MXML001 -> INICIANDO O PROCESSAMENTO DA OS " +AllTrim(cOsMobile) )

BEGIN SEQUENCE
	cFilAnt := cCodFil
	U_MXML003(SZG->ZG_NUMOS)
RECOVER
	CONOUT("#MXML001 -> ERRO AO PROCESSAR A OS " +AllTrim(cOsMobile))
END SEQUENCE

cHrFim := Time()
CONOUT( "#MXML001 -> FINALIZOU O PROCESSAMENTO DA OS " +AllTrim(cOsMobile) +" - TEMPO: " +ElapTime(cHrIni,cHrFim) )     
