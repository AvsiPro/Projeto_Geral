...
oDlg1      := MSDialog():New( 077,056,652,1133,"Conferência de Saída",,,.F.,,,,,,.T.,,,.T. )
	...
	oGrp1      := TGroup():New( 004,008,056,264,"Numero Pedido",oDlg1,CLR_BLACK,CLR_WHITE,.T.,.F. )
		...
		oGet1      := TGet():New( 015,042,{|u| If(Pcount()>0,cNota:=u,cNota)},oGrp1,150,008,'',{|| If(!Empty(cNota),BuscaNFe(cNota),)},CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"","",,)
	...
	If nPos > 0
		...
		If nPosc > 0
			If aList2[nPosc,04] >= nSoma 
				MsgAlert("Item já atingiu a quantidade de caixas solicitadas na nota fiscal")
			Else
				aList2[nPosc,04] += 1
