...
Default oself:product   := ''
	...
	cQuery := " SELECT D2_DOC,D2_SERIE,D2_QUANT,D2_PRCVEN,D2_CF,D2_FILIAL,D2_EMISSAO,D2_COD,D2_ITEM,X5_DESCRI,D2_BASEICM, "
	...
	While (cAliasTMP)->(!Eof())
		...
		aListAux[nAux]['quantity']		  		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_QUANT))
		aListAux[nAux]['price']	      	  		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_PRCVEN))
		aListAux[nAux]['desc_cfop']	      		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_CF+"-"+Alltrim((cAliasTMP)->X5_DESCRI)))
		aListAux[nAux]['base_icm']	      		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_BASEICM))
		aListAux[nAux]['value_icm']	      		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_VALICM))
		aListAux[nAux]['value_ipi']	      		:= Alltrim(EncodeUTF8((cAliasTMP)->D2_VALIPI))
