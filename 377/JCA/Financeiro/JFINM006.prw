...
static function EFETIVA()
	...
	IF lAP
		For nAt := 1 to len(aEfetiv)
			...
			//MOVIMENTO BANCมRIO
			IF aEfetiv[nAt,10] == "1"
				nOperacao := 4 //3=Inclusใo de Movimento "RECEBER"
				cTpmov	  := "R"
			ELSE
			...
			If SA6->(DbSeek( xFilial("SA6") + cBanco + cAgencia + cConta ))	.Or. SA6->(DbSeek( xFilial("SA6") + cBanco ))
				...
				aAdd( aDados, {"NCTBONLINE"        ,1                         ,Nil}) //1=Sim;2=Nใo
				...
				If lMsErroAuto
					MostraErro()
					//cMsgErro := MostraErro("\")
					//cMsgErro := "Erro na gera็ใo do movimento bancแrio:" + Chr(13) + cMsgErro
					//Conout(cMsgErro)
					lRet := .F.
					EXIT
				ELSE
		...
		ELSE
			FWAlertError("Nใo foi possํvel concluir a efetiva็ใo.", "Verifique a natureza se ้ de movimento bancแrio.")
		ENDIF

	else
		FWAlertError("Marque ao menos um item para efetivar", "Erro na concilia็ใo")
	endIf
...
/*
	Inverte a marca็ใo dos titulos
*/
