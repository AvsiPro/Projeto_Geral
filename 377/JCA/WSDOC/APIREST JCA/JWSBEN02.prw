...
/*
    API
    Api de criação, alteração e exclusão para manutneção PLANO DE SAUDE E ODONTOLOGICO RHL RHK e RHM
    Doc Mit   
                    ...
                    IF cOperacao $ "'1','2'"
                        ...
                        IF valtype(oContra["BENEFICIO"][nI]['CAMPOSRHL']) == "A" .OR. (alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHL']) != NIL .AND. !EMPTY(alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHL'])))//alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHL']) != NIL
                            ...
                            For nC := 1 to len(oContra["BENEFICIO"][nI]['CAMPOSRHL'])
                                aAuxRHL := {}
                                nHL := 0
                                For nHL := 1 to len(oContra["BENEFICIO"][nI]['CAMPOSRHL'][nC])
                                    cCampo := ALLTRIM(oContra["BENEFICIO"][1]['CAMPOSRHL'][nC][nHL]["CAMPO"])
                                    xConteudo := ALLTRIM(oContra["BENEFICIO"][1]['CAMPOSRHL'][nC][nHL]["CONTEUDO"])
                                    CAMPO := ""
                                    SX3->( DBSETORDER(1) )
                                    SX3->( DBSEEK( 'RHL' ) )
                        ...
                        IF valtype(oContra["BENEFICIO"][nI]['CAMPOSRHM']) == "A" .OR. (alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHM']) != NIL .AND. !EMPTY(alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHM']))) //alltrim(oContra["BENEFICIO"][nI]['CAMPOSRHM']) != NIL
                            ...
                            For nC := 1 to len(oContra["BENEFICIO"][nI]['CAMPOSRHM'])
                                aAuxRHM := {}
                                nHM := 0
                                For nHM := 1 to len(oContra["BENEFICIO"][nI]['CAMPOSRHM'][nC])
                                    cCampo := ALLTRIM(oContra["BENEFICIO"][1]['CAMPOSRHM'][nC][nHM]["CAMPO"])
                                    xConteudo := ALLTRIM(oContra["BENEFICIO"][1]['CAMPOSRHM'][nC][nHM]["CONTEUDO"])
                                    CAMPO := ""
                                        ...
                                        IF X3USO( SX3->X3_USADO) .AND. SX3->X3_CONTEXT != 'V'
                                            IF alltrim(cCampo) ==  ALLTRIM(SX3->X3_CAMPO)
                                                ...
                                                IF CAMPO == "RHM_CPF"
                                                    ...
                                                    ENDIF
                                                ENDIF
                                                aAux := {CAMPO,xConteudo,cCodAgr,nAgr}
                                                aadd(aAuxRHM,aAux)
                                            ENDIF
                                        ENDIF
                                        //CAMPO := ALLTRIM( SX3->X3_CAMPO)
    ...
    If SRA->(MsSeek(xFilial('SRA') + cMatric))
        ...
        IF cOperacao == "1" //alteração inclusão
            ...
            IF !oSubRHK:SeekLine(aSeekk) .And. !oSubRHK:IsDeleted()
                ...
                ELSE
                    lSra := .F.
                    aErroRet := {"Não foi localizado plano de saude ou adontologico para essa matricula","confira matricula tipo e codigo do fornecedor."}
                ENDIF
            ENDIF
            ...
            IF LEN(aRHL) > 0 .AND. lSra
                ...
                nHD := 0
                FOR nHD := 1 TO LEN(aRHL)
                    nPosField := AScan(aRHL[nHD], {|x| AllTrim(x[1]) == "RHL_CODIGO"})
                    cCodDep := aRHL[nHD,nPosField,2]
                    ...
                    n := 0
                    FOR n := 1 TO LEN(aRHL[nHD])
                        IF  aRHL[nHD][n][1] <> "RHL_CODIGO"
                            oSubRHL:SetValue(   aRHL[nHD][n][1]  ,  aRHL[nHD][n][2]      )
                        ENDIF
                    NEXT n
                NEXT nHD
            ENDIF
            ...
            IF LEN(aRHM) > 0 .and. lSra
                ...
                nHA := 0
                FOR nHA := 1 TO LEN(aRHM)
                    nPosField := AScan(aRHM[nHA], {|x| AllTrim(x[1]) == "RHM_CPF"})
                    cCodAgr := aRHM[nHA,nPosField,3]
                    nAgr := aRHM[nHA,nPosField,4]
                    ...
                    n := 0
                    FOR n := 1 TO LEN(aRHM[nHA])
                        oSubRHM:SetValue(  aRHM[nHA][n][1]  ,  aRHM[nHA][n][2]      )
                    NEXT n
