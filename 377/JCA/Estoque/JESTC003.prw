...
Local aRot := {}
    ...
    ADD OPTION aRot TITLE 'Incluir'    ACTION 'U_xJestc3()' OPERATION MODEL_OPERATION_INSERT ACCESS 0 //OPERATION 3
...
While !EOF()
    Aadd(aList,{TRB->ZPJ_FILIAL,TRB->ZPJ_CODBOX,TRB->ZPJ_DESCRI,TRB->ZPJ_ITEM,TRB->ZPJ_CODFER,TRB->ZPJ_DESCFE})
    Dbskip()
EndDo 
...
If !Empty(aList[oList:nAt,02])
    Aadd(aList,{strzero(len(aList)+1,3),'',''})
...
//SetKey(VK_F3, { ||lOk := ConPad1(,,,"ZPI",,,.f.)})
