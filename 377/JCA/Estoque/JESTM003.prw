...
Local aRot := {}
    ...
    ADD OPTION aRot TITLE 'Devolução'    ACTION 'u_xJestM3(1)' OPERATION MODEL_OPERATION_UPDATE ACCESS 0 //OPERATION 4
...
Local oModel   := MPFormModel():New("JESTM3")
...
oDlg1      := MSDialog():New( 092,232,661,1192,"Cadastro de Empréstimos",,,.F.,,,,,,.T.,,,.T. )
    ...
    oGrp2      := TGroup():New( 064,008,256,464,"Ferramentas",oDlg1,CLR_BLACK,CLR_WHITE,.T.,.F. )
        ...
        oGet9      := TGet():New( 075,148,{|u| If(Pcount()>0,cCodFr:=u,cCodFr)},oGrp2,060,008,'',{||inclfer(cCodFr,1)},CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"ZPI","",,)
...
If !Empty(cCodFer)
    ...
    If nOpc == 1
        If nPos == 0
            If !VldFerBox(cCodFer)
                MsgAlert("Ferramenta faz parte de uma caixa, não é possível empréstimo avulso")
                lRet := .F. 
            EndIf 
