...
±±ºPrograma  ³JCAESTR1   ºAutor  ³Microsiga           º Data ³  02/12/12   º±±
...
Local cTitulo := OemToAnsi("Solicitações de peças") 
Local cDescri := OemToAnsi("Este programa imprimi ordens de serviços.")
...
oSection2 := TRSection():New( oSection1,"Itens da Solicitação" , {"TRB","SCP"} ) 
...
Titulo := OemToAnsi("Entrega de peças") 
Titulo += Space(1)
...
Processa({|lEnd| GeraTra(oSection1:GetAdvplExp('SD2'),oSection1:GetAdvplExp('SA1'))},,OemToAnsi("Preparando Transit¢rio...")) 
...
oSection2:Cell("CAMPO02"):SetTitle(CHR(13)+ CHR(10)+OemToAnsi("Descrição")  ) 
