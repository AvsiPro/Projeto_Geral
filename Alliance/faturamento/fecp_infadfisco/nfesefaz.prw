						...
						*/
						...
						If lMvImpFecp .and. !(cCSTrib $ "40,41,50")
						   If (lValFecp .Or. lVfecpst) 
								...
								DbSetOrder(1)
								If SFT->(DbSeek((xFilial("SFT") + cChaveD2 )))
										//MARCELO AMARAL 27/10/2022 INÍCIO	
										dbSelectArea("CD2")
										CD2->(dbSetOrder(1))
										//CD2_FILIAL+CD2_TPMOV+CD2_SERIE+CD2_DOC+CD2_CODCLI+CD2_LOJCLI+
										//CD2_ITEM+CD2_CODPRO+CD2_IMP
										if CD2->(dbSeek(SFT->FT_FILIAL+SFT->FT_TIPOMOV+SFT->FT_SERIE+;
											SFT->FT_NFISCAL+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_ITEM+;
											SFT->FT_PRODUTO))
											if CD2->CD2_CST == "51"
												nValTFecp += SFT->FT_VFECPST + CD2->CD2_VFCPDI  + SFT->FT_VFECPMG + SFT->FT_VFESTMG	
												nValIFecp := SFT->FT_VFECPST + CD2->CD2_VFCPDI  + SFT->FT_VFECPMG + SFT->FT_VFESTMG					
											else
				...
				If lMvImpFecp .and. !(cCSTrib $ "40,41,50")
					If (lValFecp .Or. lVfecpst) 
						...
						If SFT->(DbSeek((xFilial("SFT") + cChaveD1 )))	
							...
							dbSelectArea("CD2")
							CD2->(dbSetOrder(2))
							//CD2_FILIAL+CD2_TPMOV+CD2_SERIE+CD2_DOC+CD2_CODFOR+CD2_LOJFOR+CD2_ITEM+
							//CD2_CODPRO+CD2_IMP 
							if CD2->(dbSeek(SFT->FT_FILIAL+SFT->FT_TIPOMOV+SFT->FT_SERIE+;
								SFT->FT_NFISCAL+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_ITEM+;
								SFT->FT_PRODUTO))
